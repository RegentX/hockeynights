  ...
  return (
    <div data-testid={testId('iq', 'test-card', 'card', test.id)}>
      <IceCard padding="m" className="iq-test-card">
        <div className="iq-test-card__meta" data-testid={testId('iq', 'test-card', 'panel', 'meta', test.id)}>
          <ScoreboardText data-testid={testId('iq', 'test-card', 'text', 'category', test.id)}>
            {test.category.toUpperCase()}
          </ScoreboardText>
          <Text color="secondary" data-testid={testId('iq', 'test-card', 'text', 'difficulty', test.id)}>
            {test.difficulty}
          </Text>
        </div>
        <Text variant="subheader-2" data-testid={testId('iq', 'test-card', 'text', 'title', test.id)}>
          {test.title}
        </Text>
