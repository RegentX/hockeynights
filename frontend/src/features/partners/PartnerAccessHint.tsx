...
export function PartnerAccessHint({kind}: PartnerAccessHintProps) {
  ...
  return (
    <IceCard padding="m" className="partner-cabinet-banner partner-cabinet-banner--hint">
      <div className="hockey-stack hockey-stack--gap-8" data-testid={testId('partners', 'access-hint', 'panel', kind)}>
        <Text variant="subheader-2" data-testid={testId('partners', 'access-hint', 'text', 'title', kind)}>
          Демо кабинета {label}
        </Text>
        <Text color="secondary" data-testid={testId('partners', 'access-hint', 'text', 'hint', kind)}>
        ...
        <Link to="/" data-testid={testId('partners', 'access-hint', 'link', 'login', kind)}>
          <HockeyButton view="outlined" size="s" data-testid={testId('partners', 'access-hint', 'btn', 'login', kind)}>
