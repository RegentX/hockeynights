...
export function MockShopPortalModal({open, onClose, shop}: MockShopPortalModalProps) {
  return (
    ...
    >
      <div className="hockey-stack hockey-stack--gap-8" data-testid={testId('shops', 'portal-modal', 'panel', shop.id)}>
        <Text data-testid={testId('shops', 'portal-modal', 'text', 'city', shop.id)}>
          {shop.city ?? 'Москва'}
        </Text>
        <Text color="secondary" data-testid={testId('shops', 'portal-modal', 'text', 'categories', shop.id)}>
          Категории: {shop.categories.join(', ') || 'не указаны'}
        </Text>
        <Text color="secondary" data-testid={testId('shops', 'portal-modal', 'text', 'status', shop.id)}>
          Статус: {shop.partnerStatus}
        </Text>
        <Text color="secondary" data-testid={testId('shops', 'portal-modal', 'text', 'hint', shop.id)}>
