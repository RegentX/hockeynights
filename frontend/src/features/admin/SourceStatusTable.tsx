...
export function SourceStatusTable({items}: SourceStatusTableProps) {
  ...
  return (
    <Card view="outlined" className="hockey-panel" data-testid={testId('admin', 'source-table', 'table')}>
      <div className="hockey-stack hockey-stack--gap-12">
        {items.map((item) => (
          ...
          >
            <Text color="secondary" data-testid={testId('admin', 'source-table', 'cell', 'type', item.entityType, item.entityId)}>
              {TYPE_LABELS[item.entityType]}
            </Text>
            <Text data-testid={testId('admin', 'source-table', 'cell', 'name', item.entityType, item.entityId)}>
              {item.entityName}
            </Text>
            <span data-testid={testId('admin', 'source-table', 'cell', 'source', item.entityType, item.entityId)}>
