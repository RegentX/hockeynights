...
import type {AnnotationType, HighlightCommentTag, HighlightDetail} from '@/entities/highlight'
import {AnnotationLayer} from '@/features/highlights/ui/AnnotationLayer'
import {HighlightComments} from '@/features/highlights/ui/HighlightComments'
...
export function HighlightVideoBoard({
  ...
  return (
    <div
      ...
      data-testid={testId('highlights', 'video-board', 'video', highlight.id)}
    >
      <MockUploadNotice />

      <div
        className="video-board__head"
        data-testid={testId('highlights', 'video-board', 'panel', 'head', highlight.id)}
      >
        <Text
          variant="header-2"
          data-testid={testId('highlights', 'video-board', 'text', 'title', highlight.id)}
        >
          {highlight.title}
        </Text>
        <Text
          color="secondary"
          data-testid={testId('highlights', 'video-board', 'text', 'meta', highlight.id)}
        >
      ...
      <div
        className="video-board__layout"
        data-testid={testId('highlights', 'video-board', 'panel', 'layout', highlight.id)}
      >
        <div
          className="video-board__main"
          data-testid={testId('highlights', 'video-board', 'panel', 'main', highlight.id)}
        >
          <div
          ...
          >
            <div
              className="video-board__rink"
              data-testid={testId('highlights', 'video-board', 'panel', 'rink', highlight.id)}
            />
            <AnnotationLayer
          ...
          >
            <span
              className="video-board__scrubber-label"
              data-testid={testId(
                'highlights',
                'video-board',
                'text',
        ...
        <aside
          className="video-board__aside"
          data-testid={testId('highlights', 'video-board', 'panel', 'aside', highlight.id)}
        >
          <HighlightComments
